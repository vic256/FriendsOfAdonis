...
```ts title="app/cockpit/fields/index.ts"
...
<Callout>
  Read more about the `Field` class on the dedicated [Field Reference
  page](../references/field.md).
</Callout>
...
```vue title="app/cockpit/color_picker/form.vue"
...
<Callout>
  The `InferSerializable` utility type is used to retrieve the return type of
  the `toJSON` method of your field.
</Callout>
...
export default class ProductResource extends ModelResource {
  fields() {
    return [Id.make("id"), Text.make("name"), ColorPicker.make("color").rgb()];
  }
}
```
